...
/*        public ProfileConfigurationWindow(ProfileManager p, Control sPanel, Gettable<TSettings> s, string initialProfile)
        ...
        {
            string profileName = InputBox.Show("請輸入設定檔名稱", "請輸入設定檔名稱", "");
            if (profileName == null)
                return;
            ...
            if (byName(profileName) != null)
                MessageBox.Show("抱歉，設定檔名稱必須不同", "設定檔名稱重複", MessageBoxButtons.OK);
            else
            {
        ...
        private void ProfileConfigurationWindow_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                switch (MessageBox.Show("設定檔已變更，要更新選擇的設定檔嗎? (點選「否」則會儲存至 scratchpad)",
                    "更新設定檔", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                {
