...
namespace MeGUI.core.gui
{
    public partial class AutoUpdateServerConfigWindow : Form
    {
        ...
        private void addServerButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("僅支持 Http 伺服器", "不是 Http 伺服器", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (serverList.Items.Contains(serverName))
            {
                MessageBox.Show("未添加任何東西，伺服器已經存在", "伺服器已經存在", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            foreach (string server in serverLists[0])
            {
                ...
                {
                    MessageBox.Show("官方伺服器不能添加至自訂義伺服器", "錯誤的伺服器", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
            }
