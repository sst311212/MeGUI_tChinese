...
namespace MeGUI
{
    public class UpdateHandler
    {
        ...
        private void WriteUpdateStatus()
        {
            ...
            if (iUpdatesCount > 1)
                AddTextToLog(string.Format("這裡有 {0} 個檔案可供更新: " + text.Substring(0, text.Length - 2), iUpdatesCount), ImageType.Information, false);
            else if (iUpdatesCount == 1)
                AddTextToLog("這裡有 1 個檔案可供更新: " + text.Substring(0, text.Length - 2), ImageType.Information, false);
            else
                AddTextToLog("沒有可供更新的檔案", ImageType.Information, false);
        ...
        public UpdateWindow.ErrorState DownloadFile(string url, string fileName, string packageName)
        {
            ...
            lock (this)
            {
                ...
                wc.DownloadProgressChanged += delegate(object sender, DownloadProgressChangedEventArgs e)
                {
                    ...
                    {
                        string text = string.Format("正在下載 {0} - {1} kb / {2} kb", packageName, (e.BytesReceived / 1024d).ToString("##,#"), (e.TotalBytesToReceive / 1024d).ToString("##,#"));
                ...
                if (_updateWindow != null)
                    _updateWindow.SetProgressBar(0, 1, 0, "正在下載 " + packageName);
                sw.Start();
        ...
        public UpdateWindow.ErrorState ExtractFile(MeGUI.UpdateWindow.iUpgradeable file)
        {
            ...
            {
                AddTextToLog(string.Format("無法建立 {0} 目錄.", filepath), ImageType.Error, true);
                return UpdateWindow.ErrorState.CouldNotSaveNewFile;
            }
            ...
            if (file.AvailableVersion.Url.EndsWith(".7z"))
            {
                try
                {
                    using (var oArchive = new SevenZipExtractor(Path.Combine(MainForm.Instance.Settings.MeGUIUpdateCache, Path.GetFileName(file.AvailableVersion.Url))))
                    {
                        ...
                        {
                            if (_updateWindow != null)
                                _updateWindow.SetProgressBar(0, 100, e.PercentDone, "正在解壓縮 " + file.DisplayName);
                        };
                ...
                {
                    AddTextToLog("無法解壓縮 " + file.Name + ".正在刪除檔案. Please run updater again.", ImageType.Error, true);
            ...
            {
                AddTextToLog("無法解壓縮 " + file.Name + " 檔案.", ImageType.Error, true);
                return UpdateWindow.ErrorState.CouldNotExtract;
            }
        ...
        public void ProcessUpdate()
        {
            ...
            foreach (UpdateWindow.iUpgradeable file in UpdateData)
            {
                ...
                AddTextToLog(string.Format("{1}/{2} - 正在更新 {0} 檔案", file.DisplayName, currentFile, updateableFileCount), ImageType.Information, true);
                ...
                else
                {
                    ...
                    {
                        failedFiles.Add(file);
                        AddTextToLog(string.Format("無法下載 {0} 檔案: {1}.", file.DisplayName, EnumProxy.Create(result).ToString()), ImageType.Error, true);
                    }
                    else if ((_updateMode != UpdateMode.Automatic || !String.IsNullOrEmpty(_forcePackage)) && !_abortUpdate)
                    {
                        ...
                        if (state != UpdateWindow.ErrorState.Successful)
                        {
                            ...
                            {
                                AddTextToLog(string.Format("無法安裝 {0} 檔案: {1}.", file.DisplayName, EnumProxy.Create(state).ToString()), ImageType.Error, true);
                                failedFiles.Add(file);
                            }
                            else
            ...
            if (succeededFiles.Count > 0)
                AddTextToLog("己成功更新的檔案: " + succeededFiles.Count, ImageType.Information, true);
            if (failedFiles.Count + missingFiles.Count > 0)
            {
                if (failedFiles.Count == 0)
                    AddTextToLog("尚未成功更新的檔案: " + missingFiles.Count, ImageType.Information, true);
                else
                    AddTextToLog("尚未成功更新的檔案: " + (failedFiles.Count + missingFiles.Count), ImageType.Error, true);
            }
        ...
        private UpdateWindow.ErrorState Install(UpdateWindow.iUpgradeable file)
        {
            ...
            {
                AddTextToLog(string.Format("無法安裝 '{0}' 模組，至少需要 MeGUI 版本 {1}.", file.Name, file.RequiredBuild), ImageType.Information, true);
            ...
            {
                AddTextToLog(string.Format("無法安裝 '{0}' 模組，至少需要 .NET {1}.", file.Name, file.RequiredNET), ImageType.Warning, true);
                return UpdateWindow.ErrorState.RequirementNotMet;
            }
            ...
            AddTextToLog(string.Format("無法安裝 '{0}' 模組.", file.Name), ImageType.Error, true);
            return state;
        }
        ...
        public void ShowUpdateWindow(bool bWait, bool bAutoUpdate)
        {
            ...
            if (!_updateRunning)
            {
                ...
                AddTextToLog("更新已經在處理中 - 請稍等", ImageType.Information, true);
            ...
            if (_updateMode != UpdateMode.Disabled)
            {
                foreach (string serverName in _updateServerList)
                {
                    ...
                    if (result == UpdateWindow.ErrorState.Successful)
                    {
                        ...
                        AddTextToLog("連接到伺服器: " + _updateServerURL, ImageType.Information, false);
                        break;
                    }
                    else
                        AddTextToLog("無法使用伺服器 " + _updateServerURL + ". 原因: " + EnumProxy.Create(result).ToString(), ImageType.Information, true);
                }
            }
