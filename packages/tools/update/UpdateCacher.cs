    ...
    class UpdateCacher
    {
        ...
        public static void flushOldCachedFilesAsync(UpdateWindow.iUpgradeableCollection upgradeData)
        {
            ...
            foreach (FileInfo f in files)
            {
                ...
                {
                    f.Delete();
                    MainForm.Instance.UpdateHandler.AddTextToLog("已刪除 " + f.Name + " 快取檔案", ImageType.Information, false);
                }
            }
        ...
        public static void DeleteCacheFile(string p)
        {
            ...
            {
                MainForm.Instance.UpdateHandler.AddTextToLog("無法刪除 " + localFilename + " 檔案.", ImageType.Error, true);
            }
        }
            ...
            if (file.ToLowerInvariant().EndsWith(".7z"))
            {
                ...
                try
                {
                    using (SevenZipExtractor oArchive = new SevenZipExtractor(file))
                    {
                        ...
                        {
                            MainForm.Instance.UpdateHandler.AddTextToLog("無法解壓縮 " + file + ". 正在刪除檔案.", ImageType.Information, true);
                            DeleteCacheFile(file);
                            return false;
                        }
                    }
                ...
                {
                    MainForm.Instance.UpdateHandler.AddTextToLog("無法解壓縮 " + file + ". 正在刪除檔案.", ImageType.Error, true);
                    DeleteCacheFile(file);
                    return false;
                }
            }
