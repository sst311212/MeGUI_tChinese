...
namespace MeGUI
{
	...
	public partial class ChapterCreator : Form
	{
        ...
        private void btOutput_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.Filter = "章節檔案 (*.txt)|*.txt";
            if (rbXML.Checked)
            {
                saveFileDialog.DefaultExt = "xml";
                saveFileDialog.Filter = "Matroska 章節檔案 (*.xml)|*.xml";
            }
            else if (rbQPF.Checked)
            {
                saveFileDialog.DefaultExt = "qpf";
                saveFileDialog.Filter = "x264 qp 檔案 (*.qpf)|*.qpf";
            }
        ...
        private void btInput_Click(object sender, EventArgs e)
        {
            ...
            {
                openFileDialog.Filter = "IFO 檔案 (*.ifo)|*.ifo|MKV 檔案 (*.mkv)|*.mkv|MPLS 檔案 (*.mpls)|*.mpls|文字文件 (*.txt)|*.txt|所有支援的檔案 (*.ifo,*.mkv,*.mpls,*.txt)|*.ifo;*.mkv;*.mpls;*.txt";
            ...
            else
            {
                using (FolderBrowserDialog d = new FolderBrowserDialog())
                {
                    ...
                    d.Description = "選擇 DVD, BluRay 光碟或目錄.";
