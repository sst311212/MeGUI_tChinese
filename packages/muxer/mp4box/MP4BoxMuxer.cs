...
new JobProcessorFactory(new ProcessorFactory(init), "MP4BoxMuxer");
        ...
        {
            if (line.StartsWith("正在匯入"))
                return LineType.importing;
            if (line.StartsWith("正在寫入 ISO 檔案"))
                return LineType.writing;
            if (line.StartsWith("正在分割"))
                return LineType.splitting;
            if (line.StartsWith("正在讀取 TTXT"))
        ...
        public override void ProcessLine(string line, StreamType stream, ImageType oType)
        {
            switch (getLineType(line))
            {
                ...
                case LineType.importing:
                    ...
                    {
                        if (String.IsNullOrEmpty(su.Status) || !su.Status.Equals("正在匯入視訊軌道..."))
                        {
                            su.Status = "正在匯入視訊軌道...";
                    ...
                    {
                        if (String.IsNullOrEmpty(su.Status) || !su.Status.Equals("正在匯入音訊軌道 1..."))
                        {
                            su.Status = "正在匯入音訊軌道 1...";
                            startTime = DateTime.Now;
                        }
                    }
                    else if (trackNumber == 3 && numberOfAudioTracks > 1) // second audio track
                    {
                        if (String.IsNullOrEmpty(su.Status) || !su.Status.Equals("正在匯入音訊軌道 2..."))
                        {
                            su.Status = "正在匯入音訊軌道 2...";
                            startTime = DateTime.Now;
                        }
                    }
                    else
                    {
                        if (String.IsNullOrEmpty(su.Status) || !su.Status.Equals("正在匯入軌道..."))
                        {
                            su.Status = "正在匯入軌道...";
                            startTime = DateTime.Now;
                        }
                ...
                case LineType.writing:
                    su.PercentageDoneExact = getPercentage(line);
                    if (String.IsNullOrEmpty(su.Status) || !su.Status.Equals("正在寫入..."))
                    {
                        su.Status = "正在寫入...";
                        startTime = DateTime.Now;
                    }
